				...
				<td>
					<table width="100%" border="0" align="center" cellpadding="4" cellspacing="1" bgcolor="#aec3de">
						<tr align="left" bgcolor="#F2FDFF">
							<td colspan="17" background="images/table_header.gif">能力加分列表</td>
						</tr>
					</table>
				</td>
			</tr>
			  ...
			  <table width="100%" border="1" align="center" cellpadding="4" cellspacing="1" bgcolor="#aec3de">
					...
					<tr align="right" bgcolor="#ebf0f7">
					  ...
					  <table width="100%" border="0" align="center" cellpadding="4" cellspacing="1" >
						<tr align="left" >
							<td colspan="17">
								<form action="nenglijiafenList2.do" name="myform" method="post">
				...
				%>
                            <tr align="center" bgcolor="#FFFFFF">
                             ...
                             <td>${u.xuehao}</td>    <td>${u.xingming}</td>    <td>${u.banji}</td>    <td>${u.shifouguoyingyusiji}</td>    <td>${u.shifouguoyingyuliuji}</td>    <td>${u.shifouqudejiaoshizigezheng}</td>    <td>${u.shifouweiyingzhengruwufanxiaoxuexizhe}</td>    <td>${u.fuyiqijianshifouligong}</td>    <td>${u.qita}</td>    <td>${u.jiafenshu}</td>    
                              ...
                              <td height="45" align="center"><a class="btn btn-info btn-small" href="doUpdateNenglijiafen.do?id=${u.id }">编辑</a>  <a class="btn btn-info btn-small" href="deleteNenglijiafen.do?id=${u.id }"
...
while(RS_result.next()){
		...
		共计加分数:<%=jiafenshuz%>； 其中最高加分数：<%=zgfjiafenshu%>；最低加分数：<%=zdfjiafenshu%>；平均加分数：<%out.print(Math.round(1000*(jiafenshuz/i))/1000.0);%>；
		 ...
		 <c:if test="${page.page>1}">
              <a href="nenglijiafenList2.do?page=1" >首页</a>
             <a href="nenglijiafenList2.do?page=${page.page-1 }"> 上一页</a>             </c:if>
    	     <c:if test="${page.page<page.totalPage}">
			<a href="nenglijiafenList2.do?page=${page.page+1 }">下一页</a>
			<a href="nenglijiafenList2.do?page=${page.totalPage }">末页</a>		    </c:if>		
	</c:if>
	</p>
