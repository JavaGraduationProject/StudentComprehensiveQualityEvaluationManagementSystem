...
<html xmlns="http://www.w3.org/1999/xhtml">
	...
	<table width="100%" border="0" cellpadding="0" cellspacing="0">
			  ...
			  <table width="100%" border="0" align="center" cellpadding="4" cellspacing="1" bgcolor="#aec3de">
					...
					<tr align="right" bgcolor="#ebf0f7">
					  ...
					  <table width="100%" border="0" align="center" cellpadding="4" cellspacing="1" >
						<tr align="left" >
							<td colspan="17">
								<form action="zonghecepingList.do" name="myform" method="post">
				...
				%>
                            <tr align="center" bgcolor="#FFFFFF">
                             ...
                             <td>${u.xuehao}</td>    <td>${u.xingming}</td>    <td>${u.banji}</td>    <td>${u.jisuanchuxueshengcaoxingcepingdefen}</td>    <td>${u.zhiyucepingdefen}</td>    <td>${u.nenglijiafenxiangdefen}</td>    <td>${u.jizonghedefen}</td>    
                              <td align="center"> ${u.addtime } </td>
                              <td align="center"> <!--lianjie1--> <a class="btn btn-info btn-small" href="zonghecepingDetail.do?id=${u.id}">详细</a></td>
                            </tr>
                          </c:forEach>
                        </table></td>
						</tr>
		 ...
		 <%
			String sql="select * from zongheceping where 1=1 ";
...
double jizonghedefenz=0;
							double zgfjizonghedefen=0;double zdfjizonghedefen=10000;
							
			ResultSet RS_result=connDbBean.executeQuery(sql);
while(RS_result.next()){
				
						
						jizonghedefenz=jizonghedefenz+Float.valueOf(RS_result.getString("jizonghedefen")).floatValue();
						
						if(Float.valueOf(RS_result.getString("jizonghedefen")).floatValue()>zgfjizonghedefen){zgfjizonghedefen=Float.valueOf(RS_result.getString("jizonghedefen")).floatValue();}						if(Float.valueOf(RS_result.getString("jizonghedefen")).floatValue()<zdfjizonghedefen){zdfjizonghedefen=Float.valueOf(RS_result.getString("jizonghedefen")).floatValue();}						
			 }
			%>
		共计及综合得分:<%=jizonghedefenz%>； 其中最高及综合得分：<%=zgfjizonghedefen%>；最低及综合得分：<%=zdfjizonghedefen%>；平均及综合得分：<%out.print(Math.round(1000*(jizonghedefenz/i))/1000.0);%>；
		
			<p align="center" class="fy"> <c:if test="${sessionScope.p==1 }">
		 <c:if test="${page.page>1}">
              <a href="zonghecepingList.do?page=1" >首页</a>
             <a href="zonghecepingList.do?page=${page.page-1 }"> 上一页</a>             </c:if>
    	     <c:if test="${page.page<page.totalPage}">
			<a href="zonghecepingList.do?page=${page.page+1 }">下一页</a>
			<a href="zonghecepingList.do?page=${page.totalPage }">末页</a>		    </c:if>		
	</c:if>
	</p>
